...
Return the Unix timestamp for the **end of the calendar day** (23:59:59 UTC) when the input is a date-only **string**, or the exact Unix timestamp for every other input (strings with a time component and all `datetime` objects).

```python
def end_of_day_timestamp(ts: datetime | str) -> int: ...
```

This is the single definition of "a date-only string means the whole day". `get_historical_ohlcv()` and `BatchDownloadRequest` apply it to `end` at the API boundary, *before* the range is sent to TradingView; everything downstream — `build_range_param()`, segmentation, the client-side filter — works with exact timestamps.
...
end_of_day_timestamp(datetime(2025, 12, 31, tzinfo=UTC))
                                             # 1767139200  (unchanged — a datetime is exact, even at midnight)
```
...
| `end` | `datetime \| str` | required | End of the range (inclusive). Same accepted types as `start`. Used exactly — no date-only expansion here. |
...
No date-only expansion happens in this function: `build_range_param("2024-06-15", "2024-06-15")` is a single instant. `get_historical_ohlcv()` expands a date-only `end` to 23:59:59 with `end_of_day_timestamp()` *before* calling it, which is what makes a same-day request cover the whole trading day.
...
| `ValueError` | String cannot be parsed as ISO 8601 |
| `ValueError` | `start > end` (equal values are valid — a single instant) |
