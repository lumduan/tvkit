...
# Immutable (frozen=True) singleton used when no config is supplied by the caller.
# Safe to share across calls because NormalizationConfig is frozen.
_DEFAULT_CONFIG: NormalizationConfig = NormalizationConfig()
...
def normalize_symbol(
    ...
    *,
    ...
    4. If no ``:`` is present and exactly one ``-`` is present: replace the ``-`` with ``:``
    5. Validate against ``^[A-Z0-9_]+:[A-Z0-9._!]+$`` — raise if the pattern does not match
    6. Return the canonical string

    Supported symbol formats (Phase 1 — exchange-aware inputs only):
    ...
    Bare tickers (e.g. ``"AAPL"`` with no exchange prefix) always raise
    ``SymbolNormalizationError`` in Phase 1. Default-exchange resolution is a Phase 2 feature.

    Args:
        symbol: TradingView symbol string in any supported variant. Must be a ``str``.
        config: Optional normalization configuration. Defaults to
            ``NormalizationConfig()`` (strip_whitespace=True, default_exchange=None).
    ...
    cfg: NormalizationConfig = config if config is not None else _DEFAULT_CONFIG
