...
def save_csv_file(data, **kwargs):
    ...
    **kwargs : dict
    ...
    ensure_export_directory(os.path.dirname(output_path))  # Ensure the directory exists
    try:
        # Convert data to Polars DataFrame and save as CSV
        if isinstance(data, list) and data:
            # If data is a list of dictionaries, create DataFrame from it
            df = pl.DataFrame(data)
        elif isinstance(data, dict):
            # If data is a single dictionary, convert to list first
            df = pl.DataFrame([data])
        else:
            raise ValueError("Data must be a list of dictionaries or a single dictionary")
