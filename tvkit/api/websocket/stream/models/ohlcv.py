...
class OHLCVResponse(BaseModel):
    ...
    This model parses the complete response structure from TradingView's WebSocket API.
    Handles 'du' (data update) message types with real-time OHLCV updates.
    """
    model_config = ConfigDict(extra="allow")

    message_type: str = Field(alias="m", description="Message type ('du' for data update)")
    parameters: List[Any] = Field(alias="p", description="Message parameters containing session ID and data")

    @field_validator("message_type")
    @classmethod
    def validate_message_type(cls, v: str) -> str:
        """Validate that message type is a data update."""
        if v != "du":
            raise ValueError(f"Expected message type 'du', got '{v}'")
        return v
