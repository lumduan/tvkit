...
class ExportConfig(BaseModel):
    ...
    auto_export_interval: Optional[int] = Field(None, description="Auto-export interval in seconds")
...
class StreamConfig(BaseModel):
    ...
    indicator_version: Optional[str] = Field(None, description="Indicator version if enabled")
    ...
    def validate_symbol_format(cls, v: List[str]) -> List[str]:
        ...
        for symbol in v:
            ...
            if len(symbol.split(':')) != 2:
                raise ValueError(f"Invalid symbol format '{symbol}'. Must have exactly one ':' separator")
        return v
    ...
    def validate_timeframe(cls, v: str) -> str:
        ...
        valid_timeframes: List[str] = ['1m', '5m', '15m', '30m', '1h', '2h', '4h', '1d', '1w', '1M']
...
class StreamerResponse(BaseModel):
    ...
    ohlcv_data: Optional[List[OHLCVData]] = Field(None, description="OHLCV data if applicable")
    trade_data: Optional[TradeData] = Field(None, description="Trade data if applicable")
    indicator_data: Optional[IndicatorData] = Field(None, description="Indicator data if applicable")
...
class RealtimeStreamData(BaseModel):
    ...
    connection_status: Literal['connected', 'disconnected', 'error'] = Field('disconnected', description="Connection status")
